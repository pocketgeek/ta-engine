...
#include "ai/ai.h"
#include "net/protocol.h"      // ta::net::Reader / Event / Command
...
bool loadReplayFile(const std::string& path, ReplayFile& out) {
    ...
    ta::net::Reader r(d.data() + 4, d.size() - 4);
    ta::net::ReplayHeader h;
    uint32_t fmt = 0, proto = 0;
    if (!ta::net::readReplayHeader(r, h, fmt, proto)) return false;
    ...
    if (proto != ta::net::kNetVersion) {
        out.error = "recorded under protocol v" + std::to_string(proto) +
                    ", this build is v" + std::to_string(ta::net::kNetVersion) +
    ...
    out.cfg.slots.resize(ta::net::kMaxSlots);
    int maxUsed = 0;
    for (int i = 0; i < ta::net::kMaxSlots; ++i) {
        ...
        const float mm = h.slotType[i] == 2
            ? ta::ai::incomeMultFor(ta::ai::difficultyFromLevel(h.slotAiLevel[i])) : 1.0f;
    ...
    for (uint32_t t = 0; t < nticks; ++t) {
        ...
        for (uint32_t i = 0; i < ne && br.ok; ++i) {
            ta::net::Event e; e.kind = ta::net::Event::Kind(br.u8()); e.player = br.u8();
...
std::string saveReplayFile(const std::string& dir, const ta::net::MpClient& mp,
                           uint64_t stampMs, uint64_t gameplayHash) {
    const auto& log = mp.replayLog();
    if (dir.empty() || log.empty()) return {};
    const ta::net::RoomView& room = mp.room();
    const ta::net::SlotInfo* slots = mp.startSlots();
